...
class Trainer:
    def __init__(self, model, loading_data, cfg, cfg_data):
        ...
        self.train_loader, self.val_loader, self.test_loader, self.restore_transform = loading_data()
    ...
    def train(self):
        ...
        while self.epoch < self.cfg.MAX_EPOCH:
            ...
            losses, MAPE, MSPE = self.run_epoch()
    ...
    def run_epoch(self):
        ...
        self.model.train()
        for idx, (img, gt) in enumerate(self.train_loader):
            img = img.cuda()
            gt = gt.squeeze().cuda()

            self.optim.zero_grad()
            out_den = self.model(img)
            out_den = out_den.squeeze()
            loss = self.criterion(out_den, gt)
            loss.backward()
            self.optim.step()

            losses.append(loss.cpu().item())
            error = torch.sum(out_den - gt, dim=(-2, -1)) / self.cfg_data.LABEL_FACTOR  # pred count - gt count
            APEs.append(torch.abs(error).item())
            SPEs.append(torch.square(error).item())

        MAPE = np.mean(APEs)  # Mean Absolute Patch Error
        MSPE = np.sqrt(np.mean(SPEs))  # Mean (Root) Squared Patch Error

        return losses, MAPE, MSPE
    ...
    def evaluate_model(self):
        ...
        with torch.no_grad():
            ...
            for idx, (img, gt) in enumerate(self.val_loader):
                img = img.cuda()
                gt = gt.squeeze().cuda()

                pred_den = self.model(img)
                pred_den = pred_den.squeeze()
                loss = self.criterion(pred_den, gt.cuda())
                losses.append(loss.cpu().item())
                pred_den = pred_den.cpu()

                pred_cnt = pred_den.sum() / self.cfg_data.LABEL_FACTOR
                ...
                if idx % self.eval_save_example_every == 0:
                    plt.imshow(pred_den, cmap=CM.jet)
    ...
    def save_eval_pics(self):
        ...
        for idx, (img, gt) in enumerate(self.val_loader):
            ...
            gt = gt.squeeze(0)  # Remove batch dim
