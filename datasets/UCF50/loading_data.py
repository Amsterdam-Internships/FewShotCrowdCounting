...
def loading_data():
    ...
    train_set = UCF50(cfg_data.DATA_PATH, train_folder, 'train', main_transform=train_main_transform,
                      img_transform=img_transform, gt_transform=gt_transform)
    train_loader = DataLoader(train_set, batch_size=cfg_data.TRAIN_BATCH_SIZE, num_workers=8, collate_fn=SHHA_collate,
                              shuffle=True, drop_last=True)

    val_set = UCF50(cfg_data.DATA_PATH, map(int, str(val_folder)), 'test', main_transform=None,
                    img_transform=img_transform, gt_transform=gt_transform)
