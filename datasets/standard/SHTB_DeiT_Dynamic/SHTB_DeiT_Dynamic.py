...
class SHTB_DeiT_Dynamic(data.Dataset):
    def __init__(self, data_path, mode, crop_size,
                 main_transform=None, img_transform=None, gt_transform=None, scaler=None, cropper=None):
    ...
    def __getitem__(self, index):
        ...
        img, den, mat = self.read_image_and_gt(img_path)

        if self.scaler is not None:
            img, den, _ = self.scaler(img, mat, den)
    ...
    def read_image_and_gt(self, img_path):
        ...
        gt_path = img_path.replace('img', 'gt').replace(self.img_extension, '.mat')
